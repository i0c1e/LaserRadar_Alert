...
static HPS3D_StatusTypeDef connectDevice(char* ip_addr) {
    HPS3D_StatusTypeDef ret = HPS3D_RET_OK;
    std::cout<< ip_addr << std::endl;
    ret = HPS3D_EthernetConnectDevice((char *) ip_addr, 12345, &g_handle);
//    if (ret != HPS3D_RET_OK) {
//        printf("connect failed,Err:%d\n", ret);
//    }
...
int main() {
    ...
    std::cout<<ip_addr<<std::endl;
    HPS3D_StatusTypeDef ret = connectDevice(ip_addr);
    if (ret != HPS3D_RET_OK) {
        printf("connect failed,Err:%d\n", ret);
        return 0;
    }
...
extern "C" {
void myConnectDevice(char* ip_addr) {
    connectDevice(ip_addr);
}
void myCapture() {
    capture();
}
